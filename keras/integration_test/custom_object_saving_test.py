...
class CustomObjectSavingTest(tf.test.TestCase):
  ...
  def test_register_keras_serializable_correct_class(self):
    ...
    self.assertIsInstance(loaded_model.optimizer, MyAdam)
    self.assertIsInstance(loaded_model.compiled_loss._losses[0], MyLoss)
    self.assertIsInstance(loaded_model.compiled_metrics._metrics[0], MyMetric)
    self.assertIs(loaded_model.compiled_metrics._metrics[1], my_sq_diff)
    self.assertIsInstance(loaded_model.layers[0], MyDense)
    self.assertIsInstance(loaded_model.layers[0].activity_regularizer,
                          MyRegularizer)
    self.assertIsInstance(loaded_model.layers[0].kernel_regularizer,
                          MyRegularizer)
